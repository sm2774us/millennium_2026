...
| **$\lambda$** | **Trader Risk Aversion** | Relative weight placed on variance risk vs. expected impact cost. | $\text{Currency}^{-1}$ ($\\$^{-1}$) | Higher $\lambda \implies$ higher urgency $\kappa \implies$ trade faster now. |
| **$\sigma^2$** | **Asset Price Variance Rate** | Instantaneous variance of underlying asset price returns per unit time. | $\frac{\text{Price}^2}{\text{Time}}$ | Higher $\sigma^2 \implies$ higher price risk $\implies$ higher urgency $\kappa$. |
| **$\eta$** | **Temporary Impact Parameter** | Market illiquidity coefficient penalizing squared trading velocity ($\eta \dot{x}^2$). | $\text{Currency} \cdot \frac{\text{Time}}{\text{Shares}^2}$ | Higher $\eta \implies$ illiquid market $\implies$ lowers $\kappa$, forcing smoother/slower trading. |
